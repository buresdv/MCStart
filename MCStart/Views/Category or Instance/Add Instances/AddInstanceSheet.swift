...
struct AddInstanceSheet: View
{
    ...
    var emptyInstance: Instance = .init(name: "", version: "", dateCreated: getCurrentTimeAndDate(), iconSymbolName: "", modLoader: .vanilla, mods: [], settings: InstanceSettings(javaExecutablePath: "", javaArguments: []))
    ...
    var body: some View
    {
        SheetWithHeadline(title: "Add Instance")
        {
            Form
            {
                ...
                {
                    ForEach(availableVersions, id: \.self)
                    { version in
                        Text(version)
                    }
                }
                ...
                {
                    ForEach(ModLoaders.allCases, id: \.self)
                    { modLoader in
            ...
            HStack
            {
                ...
                AdjustableLabelButton
                {
                    ...
                    else if newInstance.settings.minMemory != nil || newInstance.settings.maxMemory != nil
                    { // If the user defined any min or max memory...
